...
const NodeCoreStream = require('./node_core_stream');
...
class NodeRtmpSession extends EventEmitter {
  constructor(config, socket) {
    ...
    this.TAG = 'rtmp';
    this.config = config;
    this.bs = new NodeCoreStream(socket);
  ...
  async run() {
    do {
      try {
        ...
        let c0c1 = await this.bs.read(1537);
        let s0s1s2 = Handshake.generateS0S1S2(c0c1);
        this.bs.write(s0s1s2);
        let c2 = await this.bs.read(1536);
        console.log('[rtmp handshake] done');
      } catch (error) {
        console.log('[rtmp handshake] error', error);
        break;
      }
      ...
      try {
        ...
        while (!this.isReject) {
          ...
          let chunkBasicHeader = await this.bs.read(1);
          ...
          if (message.formatType === 0) {
            //Type 0 (11 bytes)
            chunkMessageHeader = await this.bs.read(11);
            ...
            //Type 1 (7 bytes)
            chunkMessageHeader = await this.bs.read(7);
            ...
            // Type 2 (3 bytes)
            chunkMessageHeader = await this.bs.read(3);
            message.timestampDelta = chunkMessageHeader.readUIntBE(0, 3);
            if (message.timestampDelta === 0xffffff) {
              message.extendedTimestampType = EXTENDED_TIMESTAMP_TYPE_DELTA;
            } else {
          ...
          if (message.extendedTimestampType === EXTENDED_TIMESTAMP_TYPE_ABSOLUTE) {
            let extTimestamp = await this.bs.read(4);
          ...
          let chunkBody = await this.bs.read(chunkBodySize);
          ...
          if (this.bs.readBytes >= 0xf0000000) {
            this.bs.readBytes = 0;
            this.inLastAck = 0;
          }
          if (this.ackSize > 0 && this.bs.readBytes - this.inLastAck >= this.ackSize) {
            this.inLastAck = this.bs.readBytes;
            this.sendACK(this.bs.readBytes);
          }
        }
        console.log('[rtmp message parser] done');
      } catch (error) {
        console.log('[rtmp message parser] error');
        break;
      }
    } while (0);
  ...
  reject(msg = '') {
    this.isReject = true;
    console.log(`[session ${this.id}] is rejected ${msg}`);
    this.stop();
  }
